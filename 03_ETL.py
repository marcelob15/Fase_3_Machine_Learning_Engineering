...
def plotar_comparacao_memoria(mem_raw, mem_opt, output_path):
    ...
    labels = ['CSV Bruto (Raw)', 'Parquet (Engenharia)']
...
def run_etl_com_auditoria():
    ...
    # Removemos cancelados pois não servem para prever atraso de chegada
    df = df[(df['CANCELLED'] == 0) & (df['DIVERTED'] == 0)].copy()

    # 2.4 Feature Engineering
    # Target Binária (Atraso > 15 min)
    df['TARGET'] = (df['ARRIVAL_DELAY'] > 15).astype('int8') # int8 gasta menos que int64
    
    # Extração de Hora
    df['SCHEDULED_HOUR'] = (df['SCHEDULED_DEPARTURE'] // 100).astype('int8')

    # ---------------------------------------------------------
    # 3. OTIMIZAÇÃO DE TIPOS (A MÁGICA DA ENGENHARIA)
    # ---------------------------------------------------------
    ...
    # Conversão de Objetos (Strings) para Categorias (Muito mais leve)
    cat_cols = ['AIRLINE', 'ORIGIN_AIRPORT', 'DESTINATION_AIRPORT', 'ORIGIN_STATE']
    for col in cat_cols:
        df_opt[col] = df_opt[col].astype('category')

    # Numéricos para tipos menores
    # int8: -128 a 127 (bom para mês, dia semana, target)
    # float32: Metade do tamanho de float64, precisão suficiente para lat/long/atraso
    ...
    # Limpar Nulos finais resultantes do merge
    ...
    print("="*50)
    print(f"1. Memória Original (CSV):   {memoria_antes:.2f} MB")
    print(f"2. Memória Final (Parquet):  {memoria_depois:.2f} MB")
    print(f"3. Redução de Consumo:       {100 * (1 - memoria_depois/memoria_antes):.2f}%")
    print("-" * 50)
    
    # Tabela comparativa de tipos (Para o README)
    print("\n[MUDANÇA DE TIPOS DE DADOS]")
    print(f"{'COLUNA':<25} | {'ANTES (CSV)':<15} | {'DEPOIS (PARQUET)':<15}")
    print("-" * 60)
    
    # Listar algumas colunas chave para exemplo
    exemplos = ['AIRLINE', 'MONTH', 'ARRIVAL_DELAY', 'ORIGIN_AIRPORT']
    for col in exemplos:
        t_antes = dtypes_antes.get(col, 'object') # Assumindo object se não achar
    ...
    # ---------------------------------------------------------
    # Salvar Parquet
    parquet_path = os.path.join(PARQUET_DIR, 'flights_ready.parquet')
    df_opt.to_parquet(parquet_path, index=False)
    print(f"\n💾 Arquivo Parquet salvo: {parquet_path}")
